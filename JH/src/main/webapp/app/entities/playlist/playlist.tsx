...
export const Playlist = () => {
  ...
  const playlistList = useAppSelector(state => state.playlist.entities);
            ...
            <tbody>
              {playlistList.map((playlist, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/playlist/${playlist.id}`} color="link" size="sm">
                      {playlist.id}
                    </Button>
                  </td>
                  <td>
                    {playlist.videos
                      ? playlist.videos.map((val, j) => (
                          <span key={j}>
                            <Link to={`/video/${val.id}`}>{val.id}</Link>
                            {j === playlist.videos.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
